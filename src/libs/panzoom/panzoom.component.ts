import {
  ...
  ElementRef, ViewChild, Input, NgZone
} from '@angular/core';
...
export class PanZoomComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('frameElement', { static: true })
  private frameElementRef: ElementRef;
  @ViewChild('panElement', { static: true }) private panElementRef: ElementRef;
  @ViewChild('zoomElement', { static: true })
  private zoomElementRef: ElementRef;
  @ViewChild('panzoomOverlay', { static: true })
  private panzoomOverlayRef: ElementRef;
  ...
  ngAfterViewInit(): void {
    ...
    if (this.config.dynamicContentDimensions) {
      if ((window as any).ResizeObserver) {
        this.resizeObserver = new (window as any).ResizeObserver((entries: any[]) =>
          this.onContentDimensionsChangeDetected(entries)
        );
        this.zone.runOutsideAngular(() =>
        ...
        console.error(
          'ResizeObserver API is not supported by this browser.  See https://developer.mozilla.org/en-US/docs/Web/API/ResizeObserver for info on browser compatibility.'
        );
      }
    }
    ...
    if (this.isMobileDevice()) {
      ...
      this.zone.runOutsideAngular(() => {
        ...
        this.frameElementRef.nativeElement.addEventListener(
          'wheel',
          (event: WheelEvent) => this.animationFrameFunc(() => this.onMouseWheel(event)),
          { passive: true }
        );
      });
    }
   ...
   * @param child
   */
  private isParentElement(className: string, child: { parentNode: any; }) {
    ...
    if (this.config.keepInBounds) {
      ...
      if (bottomRightCornerView.x < this.contentWidth && dragDelta.x < 0) {
        dragDelta.x *= Math.min(
          1,
          Math.pow(
            this.contentWidth - bottomRightCornerView.x,
      ...
      if (bottomRightCornerView.y < this.contentHeight && dragDelta.y < 0) {
        dragDelta.y *= Math.min(
          1,
          Math.pow(
            this.contentHeight - bottomRightCornerView.y,
            -this.config.keepInBoundsDragPullback
          )
        );
      }
  ...
  private onMouseUp = (event: MouseEvent) => {
    ...
    if (
      this.panVelocity &&
      (this.panVelocity.x !== 0 || this.panVelocity.y !== 0)
    ) {
    ...
    if (this.isMobile) {
      ...
      this.zone.runOutsideAngular(() =>
        document.removeEventListener('touchmove', this.onTouchMove, <any>{
          passive: true,
          capture: false,
        })
      );
      ...
      this.zone.runOutsideAngular(() =>
        document.removeEventListener('mousemove', this.onMouseMove, <any>{
          passive: true,
          capture: false,
        })
      );
      this.zone.runOutsideAngular(() =>
        document.removeEventListener('mouseup', this.onMouseUp, <any>{
          passive: true,
        })
      );
    }
  ...
  private animationTick = (timestamp = performance.now()) => {
    ...
    if (this.animationParams) {
      ...
      this.animationParams.progress += Math.abs(
        deltaTime / this.animationParams.duration
      );
    ...
    if (this.panVelocity && this.dragFinishing) {
      ...
      while (deltaTime > 0) {
        const dTime = Math.min(0.02, deltaTime);
    ...
    if (this.config.keepInBounds || this.dragFinishing) {
      ...
      if (topLeftCornerView.x > 0) {
        this.base.pan.x -=
          this.config.keepInBoundsRestoreForce * topLeftCornerView.x;
      }

      if (topLeftCornerView.y > 0) {
        this.base.pan.y -=
          this.config.keepInBoundsRestoreForce * topLeftCornerView.y;
      }

      if (bottomRightCornerView.x < this.contentWidth) {
        this.base.pan.x -=
          this.config.keepInBoundsRestoreForce *
          (bottomRightCornerView.x - this.contentWidth);
      }

      if (bottomRightCornerView.y < this.contentHeight) {
        this.base.pan.y -=
          this.config.keepInBoundsRestoreForce *
          (bottomRightCornerView.y - this.contentHeight);
      }
    }
  ...
  private updateDOM() {
    ...
    if (this.animationParams) {
      ...
      this.model.zoomLevel =
        this.base.zoomLevel +
        this.animationParams.deltaZoomLevel * this.animationParams.progress; // calculate how far we need to zoom in or out for the current animationTick
      const deltaTranslation = this.animationParams.panStepFunc(
        this.model.zoomLevel
      ); // calculate how far to pan the view to based on our translated coordinates
  ...
  private freeZoom(clickPoint: Point, wheelDelta: number): void {
    ...
    let newScale =
      this.scale + wheelDelta * this.config.freeMouseWheelFactor * this.scale;
    ...
    const targetPoint: Point = {
      ...
      x:
        clickPoint.x -
        (newScale / currentScale) * (clickPoint.x - currentPan.x),
      y:
        clickPoint.y -
        (newScale / currentScale) * (clickPoint.y - currentPan.y),
    };
  ...
  private getCssScale(zoomLevel: any): number {
    return Math.pow(
      this.config.scalePerZoomLevel,
      zoomLevel - this.config.neutralZoomLevel
    );
  }

  private getZoomLevel(cssScale: any) {
    return (
      Math.log10(cssScale) / Math.log10(this.config.scalePerZoomLevel) +
      this.config.neutralZoomLevel
    );
  }
  ...
  private zoomIn(zoomType: ZoomType = 'lastPoint') {
    if (zoomType === 'lastPoint') {
      this.changeZoomLevel(
        this.base.zoomLevel + this.config.zoomButtonIncrement,
        this.lastClickPoint
      );
  ...
  private getViewPosition(modelPosition: Point): Point {
    ...
    if (this.animationParams) {
      scale = this.getCssScale(
        this.base.zoomLevel +
          this.animationParams.deltaZoomLevel * this.animationParams.progress
      );
      let deltaTranslation = this.animationParams.panStepFunc(
        this.model.zoomLevel
      );
      translation = {
  ...
  private resetView() {
    if (this.config.initialZoomToFit) {
      ...
      this.config.initialPanX !== undefined &&
      this.config.initialPanY !== undefined &&
      this.config.initialZoomLevel !== undefined
    ) {
      this.zoomToFitModel({
        zoomLevel: this.config.initialZoomLevel,
        pan: {
      ...
      console.error(
        'PanZoomComponent: resetView() could not reset view as some vars were not set.  The culprits are either config.initialZoomLevel, config.initialPanX, or config.initialPanY.  Or just set panzoomConfig.initialZoomToFit'
      );
    }
  }
  ...
  private changeZoomLevel(newZoomLevel: number, clickPoint: Point) {
    ...
    const panStepFunc = (zoomLevel: number) => {
      ...
      const targetPoint: Point = {
        ...
        x:
          destPoint.x -
          (targetScale / currentScale) * (destPoint.x - currentPan.x),
        y:
          destPoint.y -
          (targetScale / currentScale) * (destPoint.y - currentPan.y),
      };
